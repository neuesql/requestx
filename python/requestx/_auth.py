...
from ._core import (
    ...
    basic_auth_header as _basic_auth_header,
    generate_cnonce as _generate_cnonce,
    compute_digest_response as _compute_digest_response,
)
...
class DigestAuth:
    ...
    def _get_client_nonce(self, nonce_count: int, nonce: bytes) -> bytes:
        ...
        # Use Rust implementation for the actual cnonce generation
        return _generate_cnonce().encode()
    ...
    def _build_auth_header(self, request, challenge):
        ...
        # Get client nonce (kept in Python for test mocking compatibility)
        ...
        # Use Rust implementation for digest response computation
        try:
            response_value, qop_value = _compute_digest_response(
                self.username,
                self.password,
                realm,
                nonce,
    ...
    def sync_auth_flow(self, request):
        ...
        if not nonce:
            raise ProtocolError(
                "Malformed Digest auth header: missing required 'nonce' field"
            )
    ...
    def sync_auth_flow(self, request):
        ...
        if self._netrc is not None:
            ...
            host = (
                url.host
                if hasattr(url, "host")
                else str(url).split("/")[2].split(":")[0].split("@")[-1]
            )
...
def _normalize_auth(auth):
    ...
    if (
        callable(auth)
        and not hasattr(auth, "sync_auth_flow")
        and not hasattr(auth, "async_auth_flow")
        and not hasattr(auth, "auth_flow")
    ):
