...
class PerformanceBenchmark:
    ...
    def benchmark_sync_library(
        self, library_func: Callable, library_name: str, num_requests: int = 50
    ) -> BenchmarkResult:
        ...
        success_rate = (
            (successful_requests / num_requests) * 100 if num_requests > 0 else 0.0
        )
        ...
        print(
            f"  {library_name}: {requests_per_second:.1f} req/s, "
            f"{avg_response_time*1000:.1f}ms avg, {success_rate:.1f}% success"
        )

        return result

    async def benchmark_async_library(
        self, library_func: Callable, library_name: str, num_requests: int = 50
    ) -> BenchmarkResult:
        ...
        print(
            f"  {library_name} (async): {requests_per_second:.1f} req/s, "
            f"{avg_response_time*1000:.1f}ms avg, {success_rate:.1f}% success"
        )
    ...
    def print_comparison_report(self):
        ...
        sorted_results = sorted(
            self.results, key=lambda x: x.requests_per_second, reverse=True
        )

        # Print table header
        print(
            f"{'Library':<20} {'RPS':<8} {'Avg(ms)':<8} {'Med(ms)':<8} {'P95(ms)':<8} "
        ...
        if len(sorted_results) > 1:
            ...
            for result in sorted_results[1:]:
                if result.requests_per_second > 0:
                    ...
                    time_improvement = (
                        (result.average_response_time - best.average_response_time)
                        / result.average_response_time
                    ) * 100

                    print(f"{best.library_name} vs {result.library_name}:")
                    print(
                        f"  Throughput: {rps_improvement:+.1f}% ({best.requests_per_second:.1f} vs {result.requests_per_second:.1f} req/s)"
                    )
                    print(
                        f"  Response Time: {time_improvement:+.1f}% ({best.average_response_time:.1f} vs {result.average_response_time:.1f} ms)"
                    )
                    print()
...
class TestPerformanceBenchmarks(unittest.TestCase):
    ...
    def test_requestx_sync_performance(self):
        ...
        result = self.benchmark.benchmark_sync_library(
            requestx_get, "RequestX", self.num_requests
        )
        ...
        self.assertLess(
            result.average_response_time, 5000
    ...
    def test_requestx_async_performance(self):
        ...
        async def run_async_benchmark():
            result = await self.benchmark.benchmark_async_library(
                requestx_get_async, "RequestX", self.num_requests
            )
            self.benchmark.results.append(result)
    ...
    def test_requests_performance(self):
        ...
        result = self.benchmark.benchmark_sync_library(
            requests_get, "requests", self.num_requests
        )
        self.benchmark.results.append(result)
    ...
    def test_httpx_sync_performance(self):
        ...
        result = self.benchmark.benchmark_sync_library(
            httpx_get, "httpx", self.num_requests
        )
        self.benchmark.results.append(result)
    ...
    def test_httpx_async_performance(self):
        ...
        async def run_async_benchmark():
            result = await self.benchmark.benchmark_async_library(
                httpx_get_async, "httpx", self.num_requests
            )
            self.benchmark.results.append(result)
    ...
    def test_aiohttp_performance(self):
        ...
        async def run_async_benchmark():
            result = await self.benchmark.benchmark_async_library(
                aiohttp_get, "aiohttp", self.num_requests
            )
            self.benchmark.results.append(result)
    ...
    def test_concurrent_performance(self):
        ...
        def concurrent_requestx_test():
            ...
            def make_request(request_id):
                try:
                    response = requestx.get(
                        f"{self.benchmark.base_url}/get?id={request_id}", timeout=30
                    )
            ...
            with ThreadPoolExecutor(max_workers=10) as executor:
                futures = [
                    executor.submit(make_request, i) for i in range(self.num_requests)
                ]
    ...
    def test_memory_efficiency(self):
        ...
        for i in range(50):
            try:
                response = requestx.get(
                    f"{self.benchmark.base_url}/get?id={i}", timeout=30
                )
...
class TestLoadTesting(unittest.TestCase):
    ...
    def test_high_concurrency_load(self):
        ...
        def worker_thread(thread_id):
            ...
            for i in range(requests_per_thread):
                try:
                    response = requestx.get(
                        f"{self.base_url}/get?thread={thread_id}&req={i}",
        ...
        print(
            f"High concurrency test: {num_concurrent} threads, {requests_per_thread} req/thread"
        )
        print(
            f"Results: {successful_requests}/{total_requests} successful ({success_rate:.1f}%)"
        )
    ...
    def test_sustained_load(self):
        ...
        while time.time() < end_time:
            try:
                response = requestx.get(
                    f"{self.base_url}/get?sustained={request_count}",
                    timeout=self.timeout,
                )
                if response.status_code == 200:
                    success_count += 1
        ...
        print(
            f"Results: {success_count}/{request_count} successful ({success_rate:.1f}%)"
        )
        ...
        self.assertGreater(success_rate, 80)  # At least 80% success rate
        self.assertGreater(
            request_count, duration_seconds * target_rps * 0.8
...
def run_full_benchmark_suite():
    ...
    async def test_requestx_async():
        ...
        result = await benchmark.benchmark_async_library(
            requestx_get_async, "RequestX", num_requests
        )
        benchmark.results.append(result)
    ...
    if HAS_REQUESTS:
        ...
        result = benchmark.benchmark_sync_library(
            requests_get, "requests", num_requests
        )
        benchmark.results.append(result)
    ...
    if HAS_HTTPX:
        ...
        async def test_httpx_async():
            ...
            result = await benchmark.benchmark_async_library(
                httpx_get_async, "httpx", num_requests
            )
            benchmark.results.append(result)
    ...
    if HAS_AIOHTTP:
        ...
        async def test_aiohttp():
            ...
            result = await benchmark.benchmark_async_library(
                aiohttp_get, "aiohttp", num_requests
            )
            benchmark.results.append(result)
