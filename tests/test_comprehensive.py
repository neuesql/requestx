...
class TestHTTPMethods(unittest.TestCase):
    ...
    def test_post_request_with_data(self):
        ...
        response = requestx.post(f"{self.base_url}/post", data=data, timeout=self.timeout)
    ...
    def test_patch_request(self):
        ...
        data = {'patch': 'value'}
        response = requestx.patch(f"{self.base_url}/patch", json=data, timeout=self.timeout)
        ...
        for method, url, data in methods_and_data:
            with self.subTest(method=method):
                if method == 'GET':
                    ...
                    response = await requestx.patch(url, json=data, timeout=self.timeout)
...
class TestResponseObject(unittest.TestCase):
    ...
    def test_response_raise_for_status(self):
        ...
        error_response = requestx.get(f"{self.base_url}/status/404", timeout=self.timeout)
...
class TestSessionManagement(unittest.TestCase):
    ...
    def test_session_post_request(self):
        ...
        response = session.post(f"{self.base_url}/post", json=json_data, timeout=self.timeout)
    ...
    def test_session_persistent_headers(self):
        ...
        if hasattr(session, 'headers'):
            ...
            data = response.json()
            if 'headers' in data:
                self.assertEqual(data['headers'].get('X-Session-Header'), 'session-value')
    ...
    def test_session_multiple_requests(self):
        ...
        for i in range(3):
            response = session.get(f"{self.base_url}/get?request={i}", timeout=self.timeout)
            self.assertEqual(response.status_code, 200)
            data = response.json()
...
class TestErrorHandling(unittest.TestCase):
    ...
    async def test_async_error_handling(self):
        ...
        # Connection error
        with self.assertRaises(requestx.ConnectionError):
            await requestx.get("https://this-domain-does-not-exist-12345.com", timeout=5)
...
class TestRequestsCompatibility(unittest.TestCase):
    ...
    def test_session_pattern(self):
        ...
        response2 = session.post(f"{self.base_url}/post", json={'test': 'data'}, timeout=self.timeout)
...
class TestIntegrationScenarios(unittest.TestCase):
    ...
    def test_authentication_scenario(self):
        ...
        # Basic auth
        response = requestx.get(f"{self.base_url}/basic-auth/user/pass", 
                               auth=('user', 'pass'), timeout=self.timeout)
        self.assertEqual(response.status_code, 200)
        data = response.json()
    ...
    def test_redirect_scenario(self):
        ...
        response = requestx.get(f"{self.base_url}/redirect/1", 
    ...
    def test_cookie_scenario(self):
        ...
        # Set cookie
        response = requestx.get(f"{self.base_url}/cookies/set/test/value", timeout=self.timeout)
        self.assertEqual(response.status_code, 200)
    ...
    def test_various_content_types(self):
        ...
        for endpoint, expected_type in content_types:
            with self.subTest(endpoint=endpoint):
                response = requestx.get(f"{self.base_url}{endpoint}", timeout=self.timeout)
                self.assertEqual(response.status_code, 200)
...
def run_async_tests():
    ...
    async def run_all_async_tests():
        ...
        async_tests = [
            ...
            ("test_async_concurrent_requests", async_http_test.test_async_concurrent_requests()),
