...
class TestExceptionHierarchy(unittest.TestCase):
    ...
    def test_all_exceptions_in_all(self):
        ...
        expected_exceptions = [
            ...
            'TooManyRedirects', 'ConnectTimeout', 'ReadTimeout', 'Timeout',
            'JSONDecodeError', 'InvalidURL', 'InvalidHeader', 'SSLError',
            'ProxyError', 'RetryError', 'UnreachableCodeError', 'InvalidSchema',
            'MissingSchema', 'ChunkedEncodingError', 'ContentDecodingError',
            'StreamConsumedError', 'FileModeWarning', 'RequestsWarning',
            'DependencyWarning'
        ]
...
class TestHeaderErrors(unittest.TestCase):
    ...
    def test_invalid_header_name(self):
        ...
        with self.assertRaises(requestx.InvalidHeader):
            requestx.get("https://httpbin.org/get", headers={"invalid\nheader": "value"})
    
    def test_invalid_header_value(self):
        """Test invalid header value raises InvalidHeader."""
        with self.assertRaises(requestx.InvalidHeader):
            requestx.get("https://httpbin.org/get", headers={"header": "invalid\nvalue"})
...
class TestErrorMessages(unittest.TestCase):
    ...
    def test_connection_error_message(self):
        ...
        try:
            ...
            self.assertTrue(
                any(word in error_msg for word in ["connection", "network", "resolve", "dns"]),
    ...
    async def run_all_async_tests():
        for test_class in async_test_classes:
            ...
            async_methods = [
                ...
                if method.startswith('test_async_') and callable(getattr(test_instance, method))
            ]
            
            for method_name in async_methods:
                success, error = await run_test_method(test_instance, method_name)
                async_results.append((f"{test_class.__name__}.{method_name}", success, error))
