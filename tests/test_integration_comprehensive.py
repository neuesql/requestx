...
class TestHTTPBinIntegration(unittest.TestCase):
    ...
    def test_get_with_query_parameters(self):
        ...
        response = requestx.get(f"{self.base_url}/get", params=params, timeout=self.timeout)
    ...
    def test_get_with_custom_headers(self):
        ...
        response = requestx.get(f"{self.base_url}/get", headers=headers, timeout=self.timeout)
    ...
    def test_post_with_form_data(self):
        ...
        response = requestx.post(f"{self.base_url}/post", data=form_data, timeout=self.timeout)
    ...
    def test_post_with_json_data(self):
        ...
        response = requestx.post(f"{self.base_url}/post", json=json_data, timeout=self.timeout)
    ...
    def test_put_request(self):
        ...
        response = requestx.put(f"{self.base_url}/put", json=put_data, timeout=self.timeout)
    ...
    def test_patch_request(self):
        ...
        response = requestx.patch(f"{self.base_url}/patch", json=patch_data, timeout=self.timeout)
    ...
    def test_status_codes(self):
        ...
        status_codes = [200, 201, 202, 204, 300, 301, 302, 400, 401, 403, 404, 500, 502, 503]
        
        for status_code in status_codes:
            with self.subTest(status_code=status_code):
                response = requestx.get(f"{self.base_url}/status/{status_code}", timeout=self.timeout)
    ...
    def test_redirects(self):
        ...
        response = requestx.get(f"{self.base_url}/redirect/1", allow_redirects=False, timeout=self.timeout)
    ...
    def test_basic_authentication(self):
        ...
        response = requestx.get(f"{self.base_url}/basic-auth/{username}/{password}",
                               auth=(username, password), timeout=self.timeout)
    ...
    def test_bearer_token_authentication(self):
        ...
        response = requestx.get(f"{self.base_url}/bearer", headers=headers, timeout=self.timeout)
    ...
    def test_cookies(self):
        ...
        response = requestx.get(f"{self.base_url}/cookies/set/{cookie_name}/{cookie_value}", 
    ...
    def test_delay_endpoint(self):
        ...
        self.assertEqual(response.status_code, 200)
        self.assertGreaterEqual(end_time - start_time, 1.0)  # Should take at least 1 second
    ...
    def test_user_agent(self):
        ...
        response = requestx.get(f"{self.base_url}/user-agent", headers=headers, timeout=self.timeout)
...
class TestAsyncIntegration(unittest.TestCase):
    ...
    async def test_async_post_request(self):
        ...
        response = await requestx.post(f"{self.base_url}/post", json=json_data, timeout=self.timeout)
    ...
    async def test_concurrent_async_requests(self):
        ...
        self.assertLess(end_time - start_time, 10)  # Should be much faster than 5+ seconds
    ...
    async def test_async_error_handling(self):
        ...
        # Test HTTP error
        response = await requestx.get(f"{self.base_url}/status/404", timeout=self.timeout)
    ...
    async def test_async_session(self):
        ...
        response1 = await session.get(f"{self.base_url}/get?session=1", timeout=self.timeout)
        response2 = await session.get(f"{self.base_url}/get?session=2", timeout=self.timeout)
    ...
    def test_invalid_url_error(self):
        ...
        for url in invalid_urls:
            with self.subTest(url=url):
                with self.assertRaises((requestx.InvalidURL, requestx.MissingSchema, TypeError)):
                    requestx.get(url)
    ...
    def test_http_error_status_codes(self):
        ...
        for code in error_codes:
            with self.subTest(status_code=code):
                response = requestx.get(f"{self.base_url}/status/{code}", timeout=self.timeout)
...
def run_async_integration_tests():
    ...
    async def run_all_async_tests():
        ...
        async_tests = [
            ...
            ("test_concurrent_async_requests", test_instance.test_concurrent_async_requests()),
