...
def generate_chart(results, output_path):
    ...
    bars1 = ax.bar([i - width/2 for i in x], rps_requestx, width, label='RequestX', color='#2ecc71')
    bars2 = ax.bar([i + width/2 for i in x], rps_requests, width, label='requests', color='#e74c3c')
    
    ax.set_xlabel('Payload Size', fontsize=12)
    ax.set_ylabel('Requests Per Second (RPS)', fontsize=12)
    ax.set_title('RequestX vs requests - POST Request Performance by Payload Size', fontsize=14)
    ax.set_xticks(x)
...
def generate_speedup_chart(results, output_path):
    ...
    colors = ['#27ae60' if s > 50 else '#f39c12' if s > 20 else '#e74c3c' for s in speedups]
...
def main():
    ...
    for name, filepath in PAYLOADS:
        ...
        speedup = ((metrics1['rps'] - metrics2['rps']) / metrics2['rps']) * 100 if metrics2['rps'] > 0 else 0
        cpu_save = ((metrics2['cpu'] - metrics1['cpu']) / metrics2['cpu']) * 100 if metrics2['cpu'] > 0 else 0
        
        print(f"\n  Results:")
        print(f"    RequestX: {metrics1['rps']:.2f} RPS | CPU: {metrics1['cpu']:.2f}% | Memory: {metrics1['mem']:.2f}MB")
        print(f"    requests: {metrics2['rps']:.2f} RPS | CPU: {metrics2['cpu']:.2f}% | Memory: {metrics2['mem']:.2f}MB")
