...
if __name__ == "__main__":
    ...
    for i, test_coro in enumerate(async_tests):
        test_name = test_coro.__name__ if hasattr(test_coro, '__name__') else f"async_test_{i}"
        try:
    ...
    # Summary
    total_passed = result.testsRun - len(result.failures) - len(result.errors) + async_passed
