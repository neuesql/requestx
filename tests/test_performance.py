...
class BenchmarkRunner:
    ...
    async def measure_async_performance(
        self, client_func, client_name: str, urls: list, timeout: float = 30.0
    ) -> PerformanceMetrics:
        ...
        rps = total_requests / total_time if total_time > 0 else 0
        avg_response_time = (
            total_time / total_requests * 1000 if total_requests > 0 else 0
        )  # ms
        success_rate = successful_requests / total_requests if total_requests > 0 else 0
        memory_usage = memory_after - memory_before
    ...
    def print_comparison_table(
        self, results: Dict[str, PerformanceMetrics], title: str
    ):
        ...
        sorted_results = sorted(
            results.items(), key=lambda x: x[1].requests_per_second, reverse=True
        )
        ...
        requestx_results = [
            result for name, result in results.items() if "requestx" in name.lower()
        ]
        if requestx_results:
            requestx_metrics = requestx_results[
                0
            ]  # Use first requestx result as baseline
            ...
            for name, metrics in sorted_results:
                if "requestx" not in name.lower():
                    rps_diff = (
                        (
                            (metrics.requests_per_second - requestx_rps)
                            / requestx_rps
                            * 100
                        )
                        if requestx_rps > 0
                    ...
                    memory_diff = (
                        (
                            (metrics.memory_usage_mb - requestx_memory)
                            / requestx_memory
                            * 100
                        )
                        if requestx_memory > 0
                        else 0
                    )
                    time_diff = (
                        (
                            (metrics.average_response_time - requestx_time)
                            / requestx_time
                            * 100
                        )
                        if requestx_time > 0
            ...
            if comparison_data:
                print(
                    tabulate(
                        comparison_data, headers=comparison_headers, tablefmt="simple"
                    )
                )
...
class TestBenchmarkComparison(unittest.TestCase):
    ...
    def test_sync_libraries_comparison(self):
        ...
        self.benchmark_runner.print_comparison_table(
            results, "Synchronous HTTP Libraries Comparison"
        )
        ...
        self.assertGreater(
            requestx_metrics.success_rate, 0.8
    ...
    def test_async_libraries_comparison(self):
        ...
        async def run_async_benchmarks():
            ...
            try:
                results["requestx_async"] = (
                    await self.benchmark_runner.measure_async_performance(
            ...
            if results:
                self.benchmark_runner.print_comparison_table(
                    results, "Asynchronous HTTP Libraries Comparison"
                )
            else:
    ...
    def test_different_http_methods_performance(self):
        ...
        if results:
            self.benchmark_runner.print_comparison_table(
                results, "HTTP Methods Performance Comparison"
            )

            # All methods should complete successfully (allow for some network issues)
            for method, metrics in results.items():
                self.assertGreaterEqual(
                    metrics.success_rate, 0.5, f"{method} had low success rate"
                )
    ...
    def test_requestx_async_vs_sync_comprehensive(self):
        ...
        for scenario_name, urls in test_scenarios:
            ...
            try:
                ...
                if async_result:
                    results[
                        f'requestx_async_{scenario_name.lower().replace(" ", "_")}'
                    ] = async_result
            except Exception as e:
            ...
            if len(results) == 2:
                self.benchmark_runner.print_comparison_table(
                    results, f"{scenario_name} - Sync vs Async"
                )
                ...
                if sync_metrics.requests_per_second > 0:
                    rps_improvement = (
                        (
                            async_metrics.requests_per_second
                            - sync_metrics.requests_per_second
                        )
                        / sync_metrics.requests_per_second
                    ) * 100
            ...
            for name, metrics in results.items():
                self.assertGreater(
                    metrics.requests_per_second, 0, f"{name} had zero RPS"
                )
                self.assertGreaterEqual(
                    metrics.success_rate, 0.8, f"{name} had low success rate"
                )
    ...
    def test_memory_efficiency_comparison(self):
        ...
        self.benchmark_runner.print_comparison_table(
            results, "Memory Efficiency Comparison"
        )

        # Verify reasonable memory usage
        for name, metrics in results.items():
            self.assertLess(
                metrics.memory_usage_mb, 100, f"{name} used too much memory"
            )  # Less than 100MB
...
class TestConcurrencyBenchmarks(unittest.TestCase):
    ...
    def test_concurrency_comparison_small(self):
        ...
        if HAS_REQUESTS:
            try:
                results["requests"] = (
                    self.benchmark_runner.measure_concurrent_performance(
                        requests.get,
                        "requests",
                        self.test_url,
                        num_requests,
        ...
        if results:
            self.benchmark_runner.print_comparison_table(
                results, f"Concurrency {concurrency} Performance"
            )
            ...
            for name, metrics in results.items():
                ...
                self.assertGreaterEqual(
                    ...
                    0.7,
                    f"{name} had low success rate: {metrics.success_rate}",
                )
    ...
    def test_concurrency_comparison_large(self):
        ...
        if results:
            self.benchmark_runner.print_comparison_table(
                results, f"Concurrency {concurrency} Stress Test"
            )
            ...
            for name, metrics in results.items():
                self.assertGreaterEqual(
                    metrics.success_rate,
                    0.3,
                    f"{name} had very low success rate: {metrics.success_rate}",
                )
        ...
        for concurrency in concurrency_levels:
            ...
            if HAS_REQUESTS:
                try:
                    ...
                    print(
                        f"Requests scaling test failed at concurrency {concurrency}: {e}"
                    )

        if scaling_results:
            self.benchmark_runner.print_comparison_table(
                scaling_results, "Concurrency Scaling Analysis"
            )
    ...
    def test_concurrent_vs_sequential_comparison(self):
        ...
        if results:
            self.benchmark_runner.print_comparison_table(
                results, "Sequential vs Concurrent Performance"
            )

            # Analyze concurrency scaling for each library
            print(f"\nConcurrency Scaling Analysis:")
            for library in ["requestx", "requests", "httpx"]:
                if (library == "requests" and not HAS_REQUESTS) or (
                    library == "httpx" and not HAS_HTTPX
                ):
                    continue

                library_results = {
                    k: v for k, v in results.items() if k.startswith(f"{library}_c")
                }
                if library_results:
                    sorted_by_concurrency = sorted(
                        library_results.items(), key=lambda x: x[1].concurrency_level
                    )
                    ...
                    best_rps = max(
                        sorted_by_concurrency, key=lambda x: x[1].requests_per_second
                    )
                    print(
                        f"Best RPS for {library.upper()}: {best_rps[1].requests_per_second:.1f} at concurrency {best_rps[1].concurrency_level}"
                    )
            ...
            for concurrency in [1, 10, 50, 100]:
                ...
                level_results = {
                    ...
                    if v.concurrency_level == concurrency
                }
                if level_results:
                    sorted_results = sorted(
                        level_results.items(),
    ...
    def test_async_concurrency_comparison(self):
        ...
        async def run_async_concurrency_test():
            ...
            if results:
                self.benchmark_runner.print_comparison_table(
                    results, "Async Concurrency Performance"
                )
    ...
    def test_sync_vs_async_requestx_comparison(self):
        ...
        if results:
            self.benchmark_runner.print_comparison_table(
                results, "RequestX Sync vs Async Performance"
            )
