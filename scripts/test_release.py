...
def test_wheel_building() -> bool:
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        exit_code, stdout, stderr = run_command([
            "uv", "run", "maturin", "build", "--release", "--strip", "--out", str(temp_path)
        ])
        if exit_code != 0:
...
def test_github_actions_syntax() -> bool:
    ...
    for workflow_file in workflow_files:
        ...
        try:
            ...
            print(f"  ⚠️  PyYAML not available, skipping syntax check for {workflow_file}")
...
def main():
    ...
    parser.add_argument(
        "--test", 
        choices=["version", "build", "quality", "tests", "wheel", "sdist", "actions", "script", "all"],
        default="all",
