...
def create_git_tag(version: str, message: str) -> bool:
    ...
    # Create tag
    exit_code, stdout, stderr = run_command(["git", "tag", "-a", tag_name, "-m", message])
    if exit_code != 0:
    ...
    try:
        if args.command == "version":
            ...
            if current != args.version:
                print(f"❌ Current version ({current}) doesn't match release version ({args.version})")
                print("Run 'python scripts/release.py bump' first or update version manually")
                sys.exit(1)
            ...
            if create_git_tag(args.version, f"Release v{args.version}"):
                ...
                print("The GitHub Actions workflow will now build and publish the release.")
            else:
